...
Axion cloud with rotating dipole background on a smooth compact-object metric.

Important:
    The current 2D RHS still uses flat derivative operators.
    The metric affects lapse in the RHS dispatch only partially through the
    flat 2D implementation, and it affects E_i B^i through index lowering.
    Full covariant 2D derivatives are planned for the next curved-2D phase.

Run with:

    python examples/run_compact_metric_rotating_dipole_2d.py
"""
...
def main() -> None:
    output_dir = ensure_output_dir(
        Path("outputs") / "phase18_compact_metric_rotating_dipole_2d"
    )
